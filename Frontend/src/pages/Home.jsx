    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          'https://technomart-peach.vercel.app/api/products'
        );
        ...
        // API request complete hone ke baad loading band
        setLoading(false);
      }
    };
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 mt-6">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <div
                key={item}
                className="bg-[#F4EAE1] rounded-2xl p-6 border border-[#ebd5b0] animate-pulse"
              >
                {/* Image Skeleton */}
                <div className="w-full h-64 bg-gray-300 rounded-xl mb-5" />
                ...
                <div className="h-5 bg-gray-300 rounded w-1/2 mb-5" />

                {/* Price Skeleton */}
                <div className="h-7 bg-gray-300 rounded w-1/3 mb-5" />
          ...
          /* No Products Found */
          <div className="text-center py-16 mt-6 bg-[#F4EAE1] rounded-2xl border border-[#ebd5b0] text-gray-500 font-bold text-xl shadow-inner">
            ...
            {filteredProducts.map((item) => (
              ...
              >
                ...
                <button
                  ...
                  className="mt-auto w-full bg-[#5d4037] hover:opacity-95 text-white font-bold py-3.5 text-base rounded-xl transition-all duration-200 active:scale-95 flex justify-center items-center gap-2 shadow-sm cursor-pointer"
                >
